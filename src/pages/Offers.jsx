...
const Offers = ({ setCreditsPage }) => {
  ...
  const handleBoardSelection = async e => {
    ...
    if (selectedBoard[0]._id) {
      ...
      return navigate(`/offers/${selectedBoard[0]._id}`);
    }
  };
  ...
  const fetchBoard = async boardId => {
    try {
      ...
      const list = await getList(offersListId);
      ...
      {loggedIn ? (
        <div>
          ...
          <div className="flex flex-wrap gap-[15px]">
            {user.boards &&
              showOffersJobs &&
              handleSort(showOffersJobs, sortBy).map((job, index) => {
